...
class AIAgentHandler:
    ...
    FTA Node JSON Structure:
    {
        ...
        "logicGate": "AND|OR" (only if type is Gate, optional for others),
    ...
    5. logicGate only valid if type is "Gate", and must be exactly "AND" or "OR"
       ("NOT" is NOT supported by the probability engine and will be rejected)
...
- The only valid logicGate values are "AND" and "OR". NOT gates are not supported
  and will be rejected; model a negation as an explicit event with its own probability"""
...
"""
                    ...
                    try:
                        ...
                        if data:
                            ...
                            if action_type in ['add', 'edit']:
                                ...
                                # 'NOT' is rejected: the probability engine treats
                                # any non-AND gate as OR, so a NOT gate would compute
                                # a silently wrong number.
                                if 'logicGate' in data:
                                    if data['logicGate'] not in ['AND', 'OR']:
                                        continue
    ...
    def _validate_node_data(self, data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        if 'logicGate' in data:
            # 'NOT' is rejected: the probability engine treats any non-AND gate as
            # OR, so a NOT gate would silently compute an OR result.
            if data['logicGate'] not in ['AND', 'OR']:
                node_label = data.get('id') or data.get('name') or '<unknown>'
                if data['logicGate'] == 'NOT':
                    return False, (
                        f"NOT gates are not supported (node '{node_label}'): the "
                        f"probability engine has no NOT semantics. Use 'AND' or 'OR'."
                    )
                return False, (
                    f"logicGate must be 'AND' or 'OR' (node '{node_label}'), "
                    f"got '{data['logicGate']}'"
                )
                    ...
                    if field in change.data:
                        try:
                            if field in ['probability']:
                                ...
                                # 'NOT' is rejected: see _validate_node_data.
                                if change.data[field] and change.data[field] not in ['AND', 'OR']:
                                    continue
                                value = change.data[field]
                            else:
                                value = change.data[field]
...
If you have specific suggestions for changes, please format them as structured proposals."""
    ...
    def verify_updated_fta_json(self, data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        def validate_node(node: Dict[str, Any], path: List[str]) -> Tuple[bool, Optional[str]]:
            ...
            # 'NOT' is rejected: FTACore._recalculate_fta_probabilities treats every
            # gate that is not "AND" as an OR gate, so a NOT gate would silently
            # produce an OR probability instead of raising.
            if lg == "NOT":
                return False, (
                    f"NOT gates are not supported (node {nid}): the probability "
                    f"engine has no NOT semantics. Use 'AND' or 'OR'."
                )
            if lg not in ["", "AND", "OR"]:
